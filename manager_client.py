...
from grpc_msgs.python3 import server_manager_pb2, server_manager_pb2_grpc



class Client():

    def __init__(self, ip, ports, lower_bound_port = 50000, upper_bound_port= 50200):
        print("startin client subroutine", ip, ports, lower_bound_port, upper_bound_port)
        assert self._connect_to_rl_srv_mng(ip,ports, lower_bound_port,upper_bound_port)
    ...
    def kill_server(self, port):
        ...
        while (i<1000):
            try:
                ...
                result = self.stub.KillServer(request= server_manager_pb2.RobotServer(port=port), timeout =240)
    ...
    def _connect_to_rl_srv_mng(self,ip,ports,lower_bound_port,upper_bound_port):
        ...
        for port in ports:
            ...
            print("success in try loop")
            return True
            #except RpcError as rpc_error:
            #    print('Client', port, RpcError)
        raise RuntimeError("Failed to connect to Server Manager")

    def _verify_connection(self):

        return self.stub.VerifyConnection(server_manager_pb2.Empty(), timeout=240).alive
